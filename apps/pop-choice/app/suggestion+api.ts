...
export async function POST(request: ExpoRequest) {
  ...
  const supabase = createClient(config.supabase.url, config.supabase.privateKey);

  const movieExpert = new MovieExpert(openai, supabase);
  const input = await request.json();
  const message = await movieExpert.recommend(input);
